...
@section('title', 'Auditoría de Duplicidad de Equipos')

@section('header-content')
    <h1 class="text-xl font-bold text-slate-800 tracking-tight">Auditoría de Duplicidad de Equipos</h1>
    ...
    <div class="w-full">
        ...
        <div class="bg-amber-50 border-l-4 border-amber-400 p-4 mb-6 rounded-r-xl shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-bold text-amber-800 uppercase tracking-tight">Reporte de Posible Duplicidad</h3>
                    <p class="text-xs text-amber-700">Este reporte identifica módulos donde se ha registrado <b>más de un equipo del mismo tipo</b> (Ej: 2 CPUs en un mismo módulo), para su revisión.</p>
                </div>
            </div>
        </div>

        {{-- Filtros --}}
        <form method="GET" action="{{ route('usuario.auditoria.duplicidad') }}" id="filterForm"
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-800">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-[10px] font-bold text-white uppercase tracking-wider text-center">Tipo Equipo</th>
                            <th class="px-6 py-4 text-[10px] font-bold text-white uppercase tracking-wider text-center">Cantidad</th>
                            <th class="px-6 py-4 text-[10px] font-bold text-white uppercase tracking-wider text-center">Inconsistencia</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-indigo-50/30 transition-all group">
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <a href="{{ route('usuario.monitoreo.modulos', $item['acta_id']) }}" target="_blank" class="font-bold text-indigo-600 hover:text-indigo-800 uppercase text-xs hover:underline">#{{ str_pad($item['numero_acta'], 5, '0', STR_PAD_LEFT) }}</a>
                                        <span class="text-[10px] font-medium text-slate-400 mt-0.5 tracking-widest">{{ \Carbon\Carbon::parse($item['fecha'])->format('d/m/Y') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-slate-600 uppercase text-[10px]">{{ $item['ipress'] }}</span>
                                        <span class="text-[9px] text-slate-400 uppercase mt-0.5">{{ $item['provincia'] }} - {{ $item['distrito'] }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="bg-slate-100 text-slate-500 px-2 py-1 rounded-lg text-[9px] font-black border border-slate-200 uppercase tracking-tighter">
                                        {{ $item['modulo_nombre'] }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="bg-amber-50 text-amber-600 px-2 py-1 rounded-lg text-[10px] font-black border border-amber-200">
                                        {{ $item['cantidad'] }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center">
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center opacity-20">
                                        ...
                                        <span class="text-xs font-black uppercase tracking-widest">¡Felicidades! No se detectaron equipos duplicados</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            provinciaSelect.addEventListener('change', async () => {
                ...
                if (provincia) {
                    const resDist = await fetch(`{{ route('usuario.auditoria.duplicidad.ajax.distritos') }}?provincia=${provincia}`);
            ...
            async function actualizarEstablecimientos(provincia, distrito) {
                establecimientoSelect.innerHTML = '<option value="">TODOS</option>';
                const resEst = await fetch(`{{ route('usuario.auditoria.duplicidad.ajax.establecimientos') }}?provincia=${provincia}&distrito=${distrito}`);
