...
<head>
  ...
  <title>Subir Fotos - {{ $titulo }}</title>
...
<body class="bg-slate-100 min-h-screen flex flex-col items-center p-4">
  ...
  <div class="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden my-4">
    <div class="bg-slate-800 p-4 text-center">
      ...
      <p class="text-indigo-300 text-sm font-bold mt-1">{{ $titulo }}</p>
    ...
    <div class="p-5 space-y-4">
      <div class="flex items-center justify-between bg-indigo-50 rounded-xl px-4 py-2.5">
        ...
        <span id="contador" class="text-sm font-black text-indigo-700">{{ count($fotosGuardadas) + count($fotosPendientes) }} / {{ $maxFotos }}</span>
      </div>

      {{-- Paso 1: tomar / elegir foto --}}
      <div id="paso_captura" class="{{ (count($fotosGuardadas) + count($fotosPendientes)) >= $maxFotos ? 'hidden' : '' }}">
      ...
      <div id="max_alcanzado" class="{{ (count($fotosGuardadas) + count($fotosPendientes)) >= $maxFotos ? '' : 'hidden' }}">
        <div class="border-2 border-dashed border-slate-300 rounded-2xl bg-slate-50 h-24 flex flex-col items-center justify-center gap-1 text-center px-4">
          <p class="text-xs font-black text-slate-500 uppercase">Máximo de {{ $maxFotos }} fotos alcanzado</p>
          <p class="text-[10px] font-bold text-slate-400">Elimine alguna pendiente de abajo para poder subir otra</p>
        </div>
      </div>

      {{-- Paso 2: preview + subir (sin descripción, este objetivo no la usa) --}}
      <div id="paso_confirmar" class="hidden space-y-3">
      ...
      <div>
        ...
        <div id="galeria_pendientes" class="space-y-2">
          @forelse ($fotosPendientes as $f)
            <div class="flex items-center gap-3 bg-amber-50 rounded-xl p-2 border border-amber-200" data-path="{{ $f['path'] }}">
              <img src="{{ asset('storage/' . $f['path']) }}" class="w-12 h-12 rounded-lg object-cover flex-shrink-0">
              <div class="min-w-0 flex-1">
                <p class="text-[9px] font-black text-amber-600 uppercase">⏳ Pendiente de guardar</p>
              </div>
              <button onclick="eliminarFoto('{{ $f['path'] }}', this)" class="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition flex-shrink-0" title="Quitar foto pendiente">
                🗑️
              </button>
            </div>
          @empty
      ...
      {{-- Ya guardadas: no se pueden borrar desde el celular --}}
      @if (count($fotosGuardadas) > 0)
        <div>
          <label class="block text-[10px] font-black text-slate-500 uppercase mb-1">Ya guardadas</label>
          <div class="space-y-2">
            @foreach ($fotosGuardadas as $path)
              <div class="flex items-center gap-3 bg-slate-50 rounded-xl p-2 border border-slate-100">
                <img src="{{ asset('storage/' . $path) }}" class="w-12 h-12 rounded-lg object-cover flex-shrink-0">
  ...
  <script>
    ...
    const MAX_FOTOS = @json($maxFotos);
    let contador = @json(count($fotosGuardadas) + count($fotosPendientes));
    ...
    const pasoConfirmar = document.getElementById('paso_confirmar');
    ...
    function actualizarEstadoCaptura() {
      const alTope = contador >= MAX_FOTOS;
    ...
    function agregarAPendientes(path, dataUrl) {
    ...
    function eliminarFoto(path, btnEl) {
      if (!confirm('¿Quitar esta foto pendiente? Todavía no estaba guardada.')) return;

      btnEl.disabled = true;
      btnEl.textContent = '...';

      fetch(`{{ url('/evidencia-movil-fijo') }}/${TOKEN}/eliminar`, {
        method: 'POST',
        headers: {
        ...
        .then(({ ok, data }) => {
          ...
          contador = data.total;
          contadorEl.textContent = `${contador} / ${MAX_FOTOS}`;
          actualizarEstadoCaptura();
    ...
    function subirFoto() {
      ...
      fetch(`{{ url('/evidencia-movil-fijo') }}/${TOKEN}/subir`, {
        method: 'POST',
        ...
        .then(({ ok, data }) => {
          ...
          contador = data.total;
          contadorEl.textContent = `${contador} / ${MAX_FOTOS}`;
          agregarAPendientes(data.path, previewImg.src);
